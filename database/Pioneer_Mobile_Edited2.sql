...
Begin
	...
	Else
		...
		Trantype varchar(2),TranNarr varchar(30),Crdr Varchar(2),Userid varchar(15))

	InSert Into t_MobTrans(ClCode, Accnr, Mytcode, MyAmount, Mydate, Mytype,Glaccount,Prevbalance,Trantype,TranNarr,Crdr,UserId)
	Select a.ClCode,'','',a.transaction_amount, cast(a.transaction_date As datetime) + cast(a.transaction_time As datetime) As MyTime, a.transaction_type,'',0,'','','', 
	b.tuserid  
	From Transactions a Inner Join Users b on a.Userid = b.userid 
	Where a.isRecorded = 0 And a.terminalId = @TerminalId And a.userId = @UserId
	...
	Select @ctcode = (Select Max(Right(Mytcode,6)) From t_MobTrans)

	Update MergeFinals..para Set varvalue = @ctcode Where varname = 'P_NEXTTCODE'
	...
	Update t_MobTrans Set Userid = Left(Rtrim(Ltrim(Userid)),2) + Right(Rtrim(Ltrim(Userid)),1)
	...
	Insert Into MergeFinals..savta(tcode,accnr,prodid,tday,type,cash,cheqid,voucher,amount,balance,penalty,commission,stationery,uid,descr)
		Select Mytcode,Accnr,Mytype,Mydate,Trantype,1,0,'MOBILE',MyAmount,Prevbalance + MyAmount,0,0,0,userid,Trantype From t_MobTrans
	...
	Insert Into t_MobTrans(ClCode,Accnr,Mytcode,MyAmount, Mydate, Mytype, Glaccount, Prevbalance,Trantype,TranNarr,Crdr,userid)
		Select ClCode,Accnr,Mytcode,MyAmount, Mydate, Mytype, '', Prevbalance,Trantype,TranNarr,'',Userid
		from t_MobTrans
	...
	Update t_MobTrans Set t_MobTrans.Glaccount =  t_products.savindacc from t_products Where t_MobTrans.Mytype = t_products.prodid
		And Isnull(t_MobTrans.Glaccount,'') = ''
	...
	Insert Into MergeFinals..genledg(Account,tcode,Tday,descriptio,Debit,credit,temp,uid,voucher,prodid,donorid,branchid,export,entrydate,lnr,accnr,trancode,costcid,cheqid)
		...
		case t_MobTrans.Crdr 
			...
			when 'DR' then 0
		end,1,UserId,'MOBILE',Mytype,'00',Left(ClCode,2),1,Getdate(),'',Accnr,'S01','000',0
	From t_MobTrans 
	Where Len(t_MobTrans.Mytype) = 3
